...
button_rects = [
    button_images[0].get_rect(center=(200, 150)),
    button_images[1].get_rect(center=(400, 150)),
    button_images[2].get_rect(center=(600, 150))
]
...
barra_alto_op = 4
barra_x_op = ancho_ventana - barra_ancho_op - 50
barra_y_op = alto_ventana - 338 - barra_alto_op

# Estado de la barra de sonido
punto_radio_music = 10
punto_x_op_music = 250
volumen_musica = 5
...
barra_alto_music = 4
barra_x_music = ancho_ventana - barra_ancho_music - 400
barra_y_music = alto_ventana - 338 - barra_alto_music
...
barra_musica = pygame.image.load("juego/imagenes/opciones/barra_musica.png")
...
barra_musica_rec = pygame.Rect(5,220,200,70)
efectos_rec = pygame.Rect(350,220,200,70)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION and event.buttons[0] == 1:
            ...
            if barra_x_music <= event.pos[0] <= barra_x_music + barra_ancho_music:
                punto_x_op_music = event.pos[0]
                volumen_musica = (punto_x_op_music - barra_x_music) / barra_ancho_music
                pygame.mixer.music.set_volume(volumen_musica)
    ...
    if pantalla_actual == "menu":
        ...
        dibujar_barra_musica()
        pantalla_opciones.blit(barra_musica,barra_musica_rec)
        ...
        mostrar_pregunta(pregunta, aciertos)
        if facil == True:
            pantalla_partida.blit(boton_pista,boton_pista_rec)
        for i, button_rect in enumerate(button_rects):
